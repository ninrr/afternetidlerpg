...
my %quest = (
    ...
    p2 => [], # point 2 for q2
#cw 21600
    qtime => time() + int(rand(3600)), # first quest starts in <=6 hours
    text => "",
    type => 1,
    stage => 1); # quest info
...
#tournaments
my @tournament; # players chosen for the tournament
my $round; # tournament round
my $battle; # tournament battle
#cw 14400 14400
my $tournamenttime = time() + 7200 + int(rand(3600)); # time to next tourney
#end tournaments

#cw tournament and quest levels
#must be HIGHER than this (4/9)
my $cwtl = 34;
my $cwql = 44;
#cw player-initiated fights
my %fights;
my $fightsrestart = time();
            ...
            if ($arg[3] eq "\1version\1") {
                ...
                if (defined $username) {
                    ...
                    if ($#arg < 6 || $arg[6] eq "") {
                        ...
                        privmsg("Be sure to check out our website at http://www.afternet.org/idle/. Thanks!", $usernick);
                        privmsg("adduser #idlerpg $usernick 100", "X3");
                        if ($opts{phonehome}) {
                ...
                # argument is optional
                    ...
                    privmsg("$arg[4]: Items: amulet: $rps{$arg[4]}{item}{amulet}".
                            "; boots: $rps{$arg[4]}{item}{'pair of boots'}".
                            "; charm: $rps{$arg[4]}{item}{charm}".
                            "; gloves: $rps{$arg[4]}{item}{'pair of gloves'}".
                            "; helm: $rps{$arg[4]}{item}{helm}".
                            "; leggings: $rps{$arg[4]}{item}{'set of leggings'}".
                            "; ring: $rps{$arg[4]}{item}{ring}".
                            "; shield: $rps{$arg[4]}{item}{shield}".
                            "; tunic: $rps{$arg[4]}{item}{tunic}".
                            "; weapon: $rps{$arg[4]}{item}{weapon}",$usernick);
                }
                    ...
                    privmsg("$username: Items: amulet: $rps{$username}{item}{amulet}".
                            "; boots: $rps{$username}{item}{'pair of boots'}".
                            "; charm: $rps{$username}{item}{charm}".
                            "; gloves: $rps{$username}{item}{'pair of gloves'}".
                            "; helm: $rps{$username}{item}{helm}".
                            "; leggings: $rps{$username}{item}{'set of leggings'}".
                            "; ring: $rps{$username}{item}{ring}".
                            "; shield: $rps{$username}{item}{shield}".
                            "; tunic: $rps{$username}{item}{tunic}".
                            "; weapon: $rps{$username}{item}{weapon}",$usernick);
                }
            }
                ...
                if (!defined($username)) {
                    ...
                    privmsg("Try: FIGHT <char name>",$usernick);
                }
                elsif (!exists($rps{$arg[4]})) {
                    privmsg("No such user.",$usernick);
                }
                elsif ($rps{$arg[4]}{online} != 1) {
                    privmsg("$arg[4] is not logged in.",$usernick);
                }
                elsif ($rps{$username} == $rps{$arg[4]}) {
                    privmsg("You may not fight yourself.",$usernick);
                }
                elsif (exists($fights{$username}{time}) && ((time() - $fights{$username}{time}) < 25200)) {
                    privmsg("You may only initiate a fight once every 7 hours. ".
                            "Time remaining: ".duration(25200 - time() + $fights{$username}{time}).".",$usernick);
                }
                elsif ((time() - $fightsrestart) < 7200) {
                    privmsg("Fights are temporarily disabled after bot restarts. Try again in a few hours!",$usernick);
                }
                else {
                    $fights{$username}{time} = time();
                    my $yousum = itemsum($username,0);
                    my $oppsum = itemsum($arg[4],0);
                    if ($yousum < 10) {
                        $yousum = 10;
                    }
                    if ($oppsum < 10) {
                    ...
                    my $youroll = int(rand($yousum));
                    my $opproll = int(rand($oppsum));
                    my $multi = 0;
                    if (($oppsum > $yousum) || ($oppsum == $yousum)) {
                        $multi = 1;
                    }
                    else {
                        $multi = 2;
                    ...
                    if ($youroll > $opproll) {
                        $result = "won";
                        $youtime = int(($youroll*60)*(($oppsum/$yousum)**$multi));
                        $opptime = int(($opproll*60)*(($oppsum/$yousum)**$multi));
                        if ($youtime > $rps{$username}{next}) {
                            $youtime = $rps{$username}{next};
                            $rps{$username}{next}=0;
                        }
                        else {
                            $rps{$username}{next} -= $youtime;
                        }
                        $rps{$arg[4]}{next} += $opptime;
                    }
                    elsif ($youroll < $opproll) {
                        $result = "lost";
                        $youtime = int(($youroll*60)*(($yousum/$oppsum)**$multi));
                        $opptime = int(($opproll*60)*(($yousum/$oppsum)**$multi));
                        if ($opptime > $rps{$arg[4]}{next}) {
                            $opptime = $rps{$arg[4]}{next};
                            $rps{$arg[4]}{next}=0;
                        }
                        else {
                            $rps{$arg[4]}{next} -= $opptime;
                    ...
                    my $messagea = "$username [$youroll/$yousum] has challenged ".
                                   "$arg[4] [$opproll/$oppsum] to a fight and $result!";
                    my $messageb;
                    if ($result eq "won") {
                        $messageb = "For reigning victorious, ".duration($youtime).
                                    " is removed from ".$username."'s time to level ".
                                    ($rps{$username}{level}+1).", and ".duration($opptime).
                                    " is added to $arg[4]'s time to level ".($rps{$arg[4]}{level}+1).".";
                    }
                    if ($result eq "lost") {
                        $messageb = "For being such a pitiful loser, ".duration($youtime).
                                    " is added to ".$username."'s time to level ".
                                    ($rps{$username}{level}+1).", and ".duration($opptime).
                                    " is removed from $arg[4]'s time to level ".($rps{$arg[4]}{level}+1).".";
                    }
                    if ($result eq "tied") {
                        $messageb = "Since this is so freaky, the Gods have awarded you both! ".
                                    duration($youtime)." is removed from ".$username."'s time to level ".
                                    ($rps{$username}{level}+1).", and ".duration($opptime).
                                    " is removed from $arg[4]'s time to level ".($rps{$arg[4]}{level}+1).".";
                    }
                    chanmsg(clog($messagea." ".$messageb));
                }
            }
        }
...
#cw 20 24 8 4
    if (!$opts{noscale}) {
        if (rand((86400/2)/$opts{self_clock}) < $online) { hog(); }
        if (rand((86400)/$opts{self_clock}) < $online) { team_battle(); }
        if (rand((2*86400)/$opts{self_clock}) < $online) { calamity(); }
        if (rand((86400/2)/$opts{self_clock}) < $online) { godsend(); }
    }
    else {
        hog() if rand(4000) < 1;
...
#cw 8 12 10
    if (rand((2*86400)/$opts{self_clock}) < $onlineevil) { evilness(); }
    if (rand((12*86400)/$opts{self_clock}) < $onlinegood) { goodness(); }
    if (rand((12*86400)/$opts{self_clock}) < 4) { war(); }
...
sub war { # let the four quadrants battle
    ...
    chanmsg(clog("A world war has taken place in the realm!"));
    ...
    for my $k (@players) {
        ...
        chanmsg(clog("War outcome: The $quadrantname[$quadrant{$k}] won, TTL of $k is halved. $k reaches next level in ".duration($rps{$k}{next}).".")) if ($iswinner[$quadrant{$k}]);
        # double ttl of users in losing quadrant
        $rps{$k}{next} *= 2 if ($isloser[$quadrant{$k}]);
        chanmsg(clog("War outcome: The $quadrantname[$quadrant{$k}] lost, TTL of $k is doubled. $k reaches next level in ".duration($rps{$k}{next}).".")) if ($isloser[$quadrant{$k}]);
    }
}
...
sub moveplayers {
    ...
    for (my $i=0;$i<$opts{self_clock};++$i) {
        ...
        if ($quest{type} == 2 && @{$quest{questers}}) {
            ...
            for (@{$quest{questers}}) {
                if ($rps{$_}{x} != $quest{"p$quest{stage}"}->[0] ||
                    $rps{$_}{y} != $quest{"p$quest{stage}"}->[1]) {
                    $allgo=0;
                    last();
                }
            }
            ...
            if ($quest{stage}==1 && $allgo) {
                ...
                undef(@{$quest{questers}});
#cw 21600
                $quest{qtime} = time() + 7200; # next quest starts in 6 hours
        ...
        delete(@temp{@{$quest{questers}}}) if (@{$quest{questers}} && $quest{type} == 2);
...
#cw 1
                if (rand($onlinecount) < 5) {
                    $positions{$rps{$player}{x}}{$rps{$player}{y}}{battled}=1;
                    collision_fight($player,$positions{$rps{$player}{x}}{$rps{$player}{y}}{user});
                }
            }
            else {
        ...
        if($quest{type} == 2 && @{$quest{questers}}) {
            for (@{$quest{questers}}) {
                if (rand(100) < 1) {
                    if ($rps{$_}{x} != $quest{"p$quest{stage}"}->[0]) {
                        $rps{$_}{x} += ($rps{$_}{x} < $quest{"p$quest{stage}"}->[0] ? 1 : -1);
                    }
                    if ($rps{$_}{y} != $quest{"p$quest{stage}"}->[1]) {
                        $rps{$_}{y} += ($rps{$_}{y} < $quest{"p$quest{stage}"}->[1] ? 1 : -1);
                    }
                }
            }
        }
...
sub quest {
    @{$quest{questers}} = grep { $rps{$_}{online} && $rps{$_}{level} > $cwql &&
    ...
    while (my $line = <Q>) {
        ...
        if ($line =~ /^Q/ && rand(++$i) < 1) {
            if ($line =~ /^Q1 (.*)/) {
                ...
                $quest{type} = 1;
#cw 43200 43201
                $quest{qtime} = time() + 14400 + int(rand(14400)); # 12-24 hours
            }
...
sub questpencheck {
    ...
    for $quester (@{$quest{questers}}) {
        if ($quester eq $k) {
            ...
            undef(@{$quest{questers}});
#cw 43200
            $quest{qtime} = time() + 3600; # 12 hours
            writequestfile();
            last;
        }
    }
    #tournaments
    #If someone gets a penalty during a tournament, then the tournament is
    #cancelled...
    for my $tourney (@tournament) {
       if ($tourney eq $k) {
          my $tourneypunish = int(0.10 * $rps{$k}{next});
          chanmsg(clog("$k has disobeyed the gods and declared himself unfit for ".
                  "tournament, therefore the remainder of the event ".
                  "shall be cancelled. ".
                  "The tournament participants have conspired to ".
                  "add ".duration($tourneypunish)." to ".$k."'s time to level ".
                  ($rps{$k}{level}+1).". Another tournament will begin within the hour!"));
          $rps{$k}{next} += $tourneypunish;
          undef @tournament;
#cw 21600 21600
          $tournamenttime = time() + 1800 + int(rand(1800));
       }
    }
    #end tournaments
}
...
#tournaments
#Pick 16 people level 31+ to participate in a royal tournament, which is
#basically a 16 player single elimination bracket type battle to the end.
#the winner gets the same reward as a HOG (but only good). The reward
#sort of sucks for people with low TTL and is uber for people with very large
#TTL's. Perhaps some balancing can be done...
sub tournament {
  return if !$opts{tournament};
  @tournament = grep { $rps{$_}{online} && $rps{$_}{level} > $cwtl &&
                      time()-$rps{$_}{lastlogin} > 14400 } keys %rps;
  if (@tournament < 16) {
#cw 21600 21600
     $tournamenttime = time() + 7200 + int(rand(3600));
     return undef @tournament;
   }
  splice(@tournament,int rand @tournament,1) while @tournament > 16;
  chanmsg(clog(join(", ",@tournament)." have been chosen by the gods to ".
          "participate in the royal tournament."));
  $round = 1;
  $battle = 1;
  tourney_yates_shuffle( \@tournament );
#cw 15
  $tournamenttime = time() + 10; #delay just a little before the next fight
}

#I'm sure the code could be cleaner, but this is it. I don't use the evil/good
#bonuses, otherwise everyone would just create a script to change their
#alignment to good when they get included in a tournament.
#Once a player loses a battle, their name is replaced with xx in the array.
#The array is packed and cleaned after each round - that way the math I use
#works.
sub tournament_battle {
   ...
   my $p1 = $battle*2-2;
   my $p2 = $p1 + 1;
   my $p1sum = itemsum($tournament[$p1]);
   my $p2sum = itemsum($tournament[$p2]);
   ...
   chanmsg(clog("Tournament Round $round, Fight $battle: $tournament[$p1] ".
      "[$p1roll/$p1sum] vs $tournament[$p2] [$p2roll/$p2sum] ... ".
      "$tournament[$winner] advances!"));

   #this removes the loser from the array
   $tournament[$loser] = "xx";
   ...
   if ($battle > (@tournament / 2)) {
      ...
      if (@tournament > 1) {
         chanmsg(clog(join(", ",@tournament)." advance to round $round of the ".
             "royal tournament."));
      }
      tourney_yates_shuffle( \@tournament );
   }
   if (@tournament == 1) {
      ...
      chanmsg(clog("$tournament[0] has won the royal tournament! As a ".
            "reward for their heroic efforts ".duration($time).
            " is removed from their time toward level ".
            ($rps{$tournament[0]}{level}+1)."."));
      $rps{$tournament[0]}{next} -= $time;
#cw 21600 21600
      $tournamenttime = time() + 7200 + int(rand(3600));
      undef @tournament;
   } else {
#cw 300
      $tournamenttime = time() + 240; # next battle in 5 minutes
   }
}
sub tourney_yates_shuffle {
